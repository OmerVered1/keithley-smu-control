...
class TimingSettingsWidget(QGroupBox):
    ...
    # bus overhead of RS-232 measurement queries (slower than USB) plus
    # general Python/Qt overhead. Without this, each iteration overshoots
    # step_size and the absolute tick alignment can never catch up.
    _AUTO_DELAY_OVERHEAD_S = 0.100  # 6430 RS-232 is slower than the USB devices
...
GRAPH_AXIS_UNITS = {
    ...
    "Current":    [("A", 1.0), ("mA", 1e3), ("µA", 1e6), ("nA", 1e9), ("pA", 1e12), ("fA", 1e15)],
    "Resistance": [("Ω", 1.0), ("kΩ", 1e-3), ("MΩ", 1e-6), ("GΩ", 1e-9)],
    "Power":      [("mW", 1e3), ("W", 1.0), ("µW", 1e6), ("nW", 1e9)],
...
class DataTableWidget(QTableWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.setHorizontalHeaderLabels(['#', 'Computer Time', 'Elapsed (s)', 'Voltage (V)', 'Current (A)', 'Resistance (Ω)', 'Power (mW)'])
    ...
    def add_point(self, point: MeasurementPoint):
        ...
        self.setItem(row, 6, QTableWidgetItem(f"{point.power*1000:.6e}" if point.power else ""))
...
class ConnectionDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        auto_detect_btn.setStyleSheet(
            "background-color: #374151; color: #e5e7eb; font-weight: 600; "
            "font-size: 14px; padding: 8px 20px; border-radius: 6px; border: none;"
        )
    ...
    def _auto_detect(self):
        ...
        self.status_label.setStyleSheet("color: #e5e7eb; font-weight: bold;")
...
class WaveToolDialog(QDialog):
    ...
    def _default_segment(self):
        return {
            "wave_type": "Sine", "avg_value": 1.0, "avg_unit": "W",
            "max_value": 2.0, "max_unit": "W", "period": 240.0,
...
class Keithley6430App(QMainWindow):
    ...
    def start_sweep(self):
        ...
        try:
            ...
            self._live_csv_writer.writerow(['Index', 'Computer_Time', 'Elapsed(s)', 'Voltage(V)', 'Current(A)', 'Resistance(Ohm)', 'Power(mW)'])
    ...
    def _build_csv_metadata_rows(self, sweep_values=None) -> List[List[str]]:
        ...
        rows.append([f"# Source: function={ss.function} mode={ss.mode.currentText()} range={ss.range.currentText() if hasattr(ss,'range') else '-'} compliance={ss.compliance.value()}"])
        guard = inst.guard_mode.isChecked() if hasattr(inst, 'guard_mode') else False
        off_mode = inst.output_off_mode.currentText() if hasattr(inst, 'output_off_mode') else '-'
        rows.append([f"# Instrument: sense={inst.sense} guard_mode={guard} output_off={off_mode}"])
    ...
    def _write_csv(self, filepath: str):
        with open(filepath, 'w', newline='') as f:
            ...
            for row in self._build_csv_metadata_rows(sweep_values):
                writer.writerow(row)
            writer.writerow(['Index', 'Computer_Time', 'Elapsed(s)', 'Voltage(V)', 'Current(A)', 'Resistance(Ohm)', 'Power(mW)'])
