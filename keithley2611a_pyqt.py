"""
Keithley 2611A Single-Channel SourceMeter Control Application
PyQt5 + pyqtgraph version with ALL features

Features:
- Modern dark theme UI with green/amber accents and pyqtgraph plotting
- Single channel (smua), connects over LAN (LXI/VXI-11), GPIB, USB, or RS-232
...
# Import our driver
from keithley2611a_driver import (
    Keithley2611A, SafetyLimits, Keithley2611AError,
...
__app_name__ = "K2611A Control Suite"
...
EXPERIMENTS_FILE = os.path.join(os.path.expanduser("~"), "Documents",
                                "K2611A_Experiments", "experiments.json")
...
class MultimeterPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.compliance.setRange(0.001, 1.5)
        ...
        self.current_range.addItems(["Auto", "100 nA", "1 µA", "10 µA", "100 µA", "1 mA", "10 mA", "100 mA", "1 A", "1.5 A"])
    ...
    def _update_source_units(self, source_type):
        if source_type == "Voltage":
            ...
            self.source_value.setRange(-1.5, 1.5)
    ...
    def start_live(self):
        ...
        try:
            ...
            if i_range == "Auto":
                ...
                range_map = {"100 nA": 100e-9, "1 µA": 1e-6, "10 µA": 10e-6, "100 µA": 100e-6, "1 mA": 1e-3, "10 mA": 10e-3, "100 mA": 100e-3, "1 A": 1, "1.5 A": 1.5}
...
class SourceSettingsWidget(QGroupBox):
    """Source settings panel — no DC/Pulse for 2611A"""
    ...
    def _setup_ui(self):
        ...
        self.range.addItems(["Auto", "100 mV", "1 V", "6 V", "40 V", "200 V"])
    ...
    def _set_function(self, func_name):
        ...
        if func_name == "Voltage":
            ...
            self.compliance.setValue(20)
            self.range.clear()
            self.range.addItems(["Auto", "100 nA", "1 \u00b5A", "10 \u00b5A", "100 \u00b5A", "1 mA", "10 mA", "100 mA", "1 A", "1.5 A"])


class InstrumentSettingsWidget(QGroupBox):
    """Instrument settings: Sense mode (no terminal for 2611A)"""
...
class MeasureSettingsWidget(QGroupBox):
    ...
    def _setup_ui(self):
        ...
        self.measure_range.addItems(["Auto", "100 nA", "1 \u00b5A", "10 \u00b5A", "100 \u00b5A", "1 mA", "10 mA", "100 mA", "1 A", "1.5 A"])
...
class ConnectionDialog(QDialog):
    ...
    def _load_lan_history(self) -> List[str]:
        if not self.app or not hasattr(self.app, "settings"):
            return []
        raw = self.app.settings.value(self._LAN_HISTORY_KEY, "")
        return [ip for ip in raw.split(",") if ip] if raw else []

    def _save_lan_history(self, ip: str):
        if not self.app or not hasattr(self.app, "settings"):
            return
        history = [x for x in self._load_lan_history() if x != ip]
        history.insert(0, ip)
        history = history[: self._LAN_HISTORY_MAX]
        self.app.settings.setValue(self._LAN_HISTORY_KEY, ",".join(history))

    def _refresh(self):
        self.resource_list.clear()
        resources = list(Keithley2611A.list_available_instruments())

        # Previously-used LAN IPs (e.g. a static-IP instrument VISA can't
        # broadcast-discover) — remembered so they don't need retyping.
        lan_resources = [f"TCPIP::{ip}::inst0::INSTR" for ip in self._load_lan_history()]
        for r in lan_resources:
            if r not in resources:
                resources.append(r)

        for r in resources:
            self.resource_list.addItem(r)
            if "2611" in r or "05E6" in r.upper():
    ...
    def _connect_lan(self):
        ...
        if not network_setup.is_reachable(ip):
            reply = QMessageBox.question(
                self, "Instrument Unreachable",
                f"Can't reach {ip} on the network.\n\n"
                "This usually happens when the instrument is on a dedicated "
                "LAN cable with a static IP and your network adapter is on "
                "a different subnet (auto-assigned 169.254.x.x).\n\n"
                "Try to auto-configure a network adapter to reach it?",
                QMessageBox.Yes | QMessageBox.No,
            )
            if reply == QMessageBox.Yes:
                if not self._auto_configure_network(ip):
                    return
            else:
                return
        ...
        try:
            ...
            self._save_lan_history(ip)
            self.accept()
        except Exception as e:
            QMessageBox.critical(self, "Connection Error", str(e))

    def _auto_configure_network(self, target_ip: str) -> bool:
        """Add a secondary IP to a network adapter so it can reach target_ip.
        Existing addresses on the adapter are left untouched. Windows only —
        other platforms get manual instructions."""
        if sys.platform != "win32":
            QMessageBox.information(
                self, "Manual Setup Required",
                f"Auto-configure is only implemented for Windows.\n\n"
                f"Assign your network adapter a static IP in the same /24 "
                f"subnet as {target_ip} (matching the first three octets)."
            )
            return False

        adapters = network_setup.list_windows_adapters()
        if not adapters:
            QMessageBox.warning(self, "No Adapters Found", "Could not enumerate network adapters.")
            return False

        labels = [f"{a['Name']}  ({a.get('IPAddress') or 'no IP'})" for a in adapters]
        choice, ok = QInputDialog.getItem(
            self, "Select Adapter",
            "Which network adapter is connected to the instrument?",
            labels, 0, False,
        )
        if not ok:
            return False

        adapter = adapters[labels.index(choice)]
        suggested_ip = network_setup.suggest_secondary_ip(target_ip)

        QMessageBox.information(
            self, "Administrator Approval Needed",
            f"Windows will now ask for administrator approval to add "
            f"{suggested_ip} to '{adapter['Name']}'. Click Yes on that prompt."
        )
        success, msg = network_setup.add_secondary_ip_windows(adapter["Name"], suggested_ip)
        if not success:
            QMessageBox.critical(self, "Auto-Configure Failed", msg)
            return False

        if network_setup.is_reachable(target_ip):
            QMessageBox.information(self, "Network Configured", f"{msg}\n\n{target_ip} is now reachable.")
            return True
        else:
            QMessageBox.warning(
                self, "Still Unreachable",
                f"{msg}\n\nBut {target_ip} still isn't responding. Double-check the "
                "instrument's LAN cable and its configured IP on the front panel."
            )
            return False
...
class SafetyDialog(QDialog):
    ...
    def _setup_ui(self):
        ...
        self.max_voltage.setRange(0.1, 200)
        ...
        self.min_voltage.setRange(-200, 0)
        ...
        self.max_current.setRange(0.001, 1.5)
        ...
        self.min_current.setRange(-1.5, 0)
        ...
        self.power_limit.setRange(0.1, 30.6)
...
class Keithley2611AApp(QMainWindow):
    """Main application window — single-channel 2611A control"""
    ...
    def __init__(self):
        ...
        self.smu: Optional[Keithley2611A] = None
        ...
        self.auto_save_path = os.path.join(os.path.expanduser("~"), "Documents", "K2611A_Data")
    ...
    def _update_output_buttons(self):
        """Update output button label and style"""
        if self.smu:
            btn = self.output_a_btn
            if self.smu.output_enabled("a"):
    ...
    def _setup_ui(self):
        ...
        self.output_a_btn = QPushButton("OUTPUT: OFF")
        ...
        # running K2611A sweep (V, I, R, P). Connection is managed from the
    ...
    def _show_about(self):
        ...
        <p>{__copyright__}</p>
        <hr>
        <p>Professional Keithley 2611A Single-Channel SMU Control and I-V Characterization Software</p>
        <p><b>Features:</b></p>
        <ul>
        <li>LAN (LXI/VXI-11), GPIB, USB, and RS-232 connectivity</li>
        ...
        if file:
            ...
            if "safety" in config:
                self.safety_limits = SafetyLimits(
                    ...
                    max_current=config["safety"].get("max_current", 1.5),
                    min_current=config["safety"].get("min_current", -1.5),
                    power_limit=config["safety"].get("power_limit", 20.0)
                )
