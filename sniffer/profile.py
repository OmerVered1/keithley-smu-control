# READ-ONLY MODULE
# Vendored from OmerVered1/lan-signal-sniffer (lan_sniffer/protocol/profile.py) — sync manually.
...
from .reassembly import C2S, S2C, StreamChunk
from .modbus import FORMATS as MODBUS_FORMATS, RegisterSpec
...
def bundled_profile_dir() -> Path:
    ...
    if frozen:
        return Path(frozen) / "calorimeter_profiles"
    return Path(__file__).resolve().parents[1] / "calorimeter_profiles"
...
def user_profile_dir() -> Path:
    ...
    return base / "Keithley SMU Control Suite" / "calorimeter_profiles"
